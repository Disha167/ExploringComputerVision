...
from skimage.metrics import structural_similarity as ssim  # to compare 2 images
...
def main(image_filename):
    ...
    blank_image = np.zeros((60, 60, 3), np.uint8, order='C')
    ...
    for (x, y, window) in traversal.sliding_window(image, stepSize=60, windowSize=(winW, winH)):
        ...
        # show this iteration
        cv2.imshow("Path Planning Algorithm using Computer Vision", clone)
        cv2.waitKey(1)
        time.sleep(0.025)
