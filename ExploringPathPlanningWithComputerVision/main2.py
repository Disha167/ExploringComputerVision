...
def main(image_filename):
    ...
    obstacles = []
    index = [1, 1]
    # create a blank image,initialised as matrix of zeroes
    ...
    for (x, y, window) in traversal.sliding_window(image, stepSize=60, windowSize=(winW, winH)):
        # if the window dose'nt meet our desired window size, ignore it
        if window.shape[0] != winH or window.shape[1] != winW:
            continue
        # print index, image is our iterator
        clone = image.copy()
        # format the square openCV
    ...
    for startimage in list_colored_grids:
        ...
        for grid in [n for n in list_colored_grids if n != startimage]:
            # if it is not equal to startimage, move to next image
            ...
            s = ssim(image, image2)
            # if they are similar, we perform Astarsearch
            if s > 0.9:
                # perform a star
    ...
    for element in occupied_grids:
        print(element)
    for element in planned_path:
        print(element, planned_path[element])
